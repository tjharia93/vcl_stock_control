...
class StockUOMConversionRule(Document):
    def validate(self):
        ...
        if self.conversion_for == "Item" and not self.item:
            frappe.throw("Item is required when Conversion For is 'Item'.")
        elif self.conversion_for == "Material Profile" and not self.material_profile:
